...
class IkaMatcher:
	...
	def FP_FRONT_IS_WHITE(self, mask_img = None, img_bgr = None, img_gray = None):
		...
		fp_img = np.minimum(mask_img, white_mask_s, white_mask_v) # マスク範囲=0, 白以外=0, 白=255
		orig_hist = cv2.calcHist([fp_img], [0], None, [3], [0, 256])
		orig_raito  = orig_hist[2] / np.sum(orig_hist)
		return orig_raito, fp_img
	...
	def match(self, img, debug = None):
		...
		orig_raito,img_fp  = self.false_positive_method(mask_img = self.mask_img, img_bgr = img_bgr, img_gray = img_gray)
		raito = 0
		...
		if match and not (self.pre_threshold_value is None):
			ret, img_gray = cv2.threshold(img_gray, self.pre_threshold_value, 255, cv2.THRESH_BINARY)
