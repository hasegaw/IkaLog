...
class IkaMatcher1(object):
    ...
    def match_score(self, img, debug=None):
        ...
        #img_bg_1d = np.reshape(img_bg, (-1))

        #print("number of pixels: %d" % img_bg_1d.shape[0])
        #print("number of black pixels: %d" % np.extract(img_bg_1d == 0, img_bg_1d).shape[0])
        #print("number of white pixels: %d" % np.extract(img_bg_1d != 0, img_bg_1d).shape[0])
        #print(orig_hist)
