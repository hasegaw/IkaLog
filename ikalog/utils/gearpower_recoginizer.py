...
class GearpowerRecoginizer(object):
    # Models
    groups = []

    # Normalize the image. (for gearpowers)
    #
    # - Crop the image
    ...
    def normalize_gearpower_image(self, img):
        h = img.shape[0]
        w = img.shape[1]
        #img = img[2:h - 4, 2:w - 4]
    ...
    def analyze_image(self, img, blocks_x=3, blocks_y=3, debug=False):
        imgs = self.normalize_gearpower_image(img)
        param = {'hist': imgs[0]}
        dimg = imgs[1]
        return param, dimg

    def show_learned_gearpower_image(self, l, name='hoge', save=None):
        if len(l) != 0:
    ...
    def name2id(self, name):
        try:
            return self.gearpower_names.index(name)
        except:
            self.gearpower_names.append(name)
        return self.gearpower_names.index(name)

    def id2name(self, id):
        return self.gearpower_names[id]
...
#                        return
    ...
    def save_model_to_file(self, file):
        f = open(file, 'wb')
        pickle.dump([self.samples, self.responses, self.gearpower_names], f)
        f.close()
    ...
    def load_model_from_file(self, file):
        ...
        self.samples = l[0]
        self.responses = l[1]
        self.gearpower_names = l[2]

    def __init__(self):
        self.gearpower_names = []
        self.knn_reset()
        self.groups = []
