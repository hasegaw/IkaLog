...
class StatInkCollector(IkaLogPlugin):
    ...
    def _get_offset_msec(self, context, msec=None):
        msec = msec or context['engine']['msec']
        if (msec and context['game'].get('start_offset_msec')):
            return msec - context['game']['start_offset_msec']
        return None
    ...
    def on_game_death_reason_identified(self, context):
        ...
        if self.last_dead_event is not None:
            ...
            self.last_dead_event['cause'] = reason
            self.last_dead_event = None
    ...
    def on_game_special_weapon(self, context, params={}):
        ...
        event_msec = params['msec']
        at = self._get_offset_msec(context,event_msec)
        if event_msec and at:
            self._add_event(context, {
                'type': 'special_activation',
                'special': params['special_weapon'],
                'at': int(at / 100) / 10,
                'me': params['me'],
            })
    ...
    def on_game_cramblitz_we_broke(self, context):
        self._add_ranked_battle_event(context, 'we_broke')

    def on_game_cramblitz_we_back(self, context):
        self._add_ranked_battle_event(context, 'we_back')

    def on_game_cramblitz_they_broke(self, context):
        self._add_ranked_battle_event(context, 'they_broke')

    def on_game_cramblitz_they_back(self, context):
        self._add_ranked_battle_event(context, 'they_back')
