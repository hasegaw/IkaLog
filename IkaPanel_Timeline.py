...
#			h2 = y2 * 1.0 * (self.gh / 8)
			...
			if not last_xPos is None:
				...
				self.dc.DrawRectangle(self.margin_dots + last_xPos, self.margin_dots, xPos - last_xPos, h2)

				self.dc.SetPen(wx.Pen(wx.YELLOW, 1))
				self.dc.SetBrush(wx.Brush('yellow'))
				self.dc.DrawRectangle(self.margin_dots + last_xPos, self.margin_dots + self.gh - h1, xPos - last_xPos,  h1)
			last_xPos = xPos
	...
	def drawTower(self, context):
		...
		for sample in context['game']['towerTrack']:
			...
			if not last_yPos is None:
				self.dc.DrawLine(self.margin_dots + last_xPos, self.margin_dots + last_yPos, self.margin_dots + xPos, self.margin_dots + yPos)
			last_xPos = xPos
			last_yPos = yPos
	...
	def drawGraphFinalize(self):
		...
		self.dc.DrawLine(self.margin_dots, self.margin_dots, self.margin_dots, self.margin_dots + self.gh)
		self.dc.DrawLine(self.margin_dots, self.margin_dots + self.gh, self.margin_dots + self.gw, self.margin_dots + self.gh)
